...
const MAX_OPENROUTER_REQUESTS_PER_DAY = 1000
...
async function callOpenRouter(prompt, maxTokens = 900) {
  ...
  const state = loadSyncState()
  if (state.openRouterUsageDate !== todayKey()) state.openRouterUsageCount = 0
  if ((state.openRouterUsageCount || 0) >= MAX_OPENROUTER_REQUESTS_PER_DAY)
    throw new Error('Daily OpenRouter limit reached')
  state.openRouterUsageDate = todayKey()
  state.openRouterUsageCount = (state.openRouterUsageCount || 0) + 1
  saveSyncState(state)
