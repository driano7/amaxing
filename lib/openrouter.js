...
export async function amaxingChat({ messages, model, maxTokens = 500, temperature = 0.4 }) {
  // OpenRouter por defecto (revertido por solicitud, Gemini no jaló). Usa GEMINI solo si AI_PROVIDER=gemini explícito
  const useGemini = GEMINI_API_KEY && AI_PROVIDER === 'gemini'
  if (useGemini) {
    try {
