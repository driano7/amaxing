...
export async function amaxingChat({ messages, model, maxTokens = 500, temperature = 0.4 }) {
  ...
  if (useGemini) {
    ...
    // Incluir proyecto para cuotas y trazabilidad (x-goog-user-project)
    ...
    // Si el contenido empieza con "*   *Revised Draft:*", limpiarlo
    if (content.includes('*Revised Draft:*')) {
      ...
      // Eliminar cualquier prefijo con * o "
      content = content.replace(/^[*"\s]+/, '').trim()
    }
    // Limpieza robusta: si el modelo expuso su razonamiento
    if (typeof content === 'string') {
      // Caso especial: el modelo a veces antepone "thought" literal
      ...
      // Eliminar comillas al inicio si quedaron
      content = content.replace(/^["']\s*/, '').trim()
    }
